...
function AddQuestionModal({ isOpen, onClose, editData }: { isOpen: boolean, onClose: () => void, editData?: any }) {
  ...
  useEffect(() => {
    if (editData) {
      ...
      if (editData.answers && editData.answers.length > 0) {
        const selectedQuestionType = editData.question_type?.name || questionTypeData?.data?.find((item: any) => item.id === questionTypeId)?.name;
  ...
  const onSubmit = async (data: any) => {
    ...
    if (selectedQuestionType?.name === 'Options') {
      answersArray = [
        { text: data.optionA || "", is_correct: data.answer === "0" },
        { text: data.optionB || "", is_correct: data.answer === "1" },
        { text: data.optionC || "", is_correct: data.answer === "2" },
        { text: data.optionD || "", is_correct: data.answer === "3" }
      ];
      ...
      answersArray = [
        { text: "True", is_correct: data.answer === "True" },
        { text: "False", is_correct: data.answer === "False" }
      ];
    } else if (selectedQuestionType?.name === 'Text') {
      answersArray = [
        { text: data.answer || "", is_correct: true }
      ];
    }
  ...
  // Create a reusable component for option input with proper event handling
  const OptionInput = useCallback(({ label, optionKey, register, t }: any) => {
    return (
      <div className="mb-4">
        ...
        <DialogContent className='max-w-[95vw] md:w-[650px] lg:max-w-[700px] max-h-[95vh] overflow-y-auto'>
          <div className="w-full ">
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              {/* Question (text input) - FIXED: Using Controller instead of register */}
              <div className="mb-4">
                <label htmlFor="question" className="block text-sm font-medium text-gray-700">{t("question")}</label>
                <Controller
                  name="question"
                  control={control}
                  rules={{ required: t("question_name_is_required") }}
                  render={({ field }) => (
                    <input 
                      {...field}
                      type="text" 
              ...
              {selectedTypeName === 'Options' && (
                <>
                  <OptionInput label={t("optionA")} optionKey="optionA" register={register} t={t} />
                  <OptionInput label={t("optionB")} optionKey="optionB" register={register} t={t} />
                  <OptionInput label={t("optionC")} optionKey="optionC" register={register} t={t} />
                  <OptionInput label={t("optionD")} optionKey="optionD" register={register} t={t} />
              ...
              {selectedTypeName === 'Text' && (
                <div className="mb-4">
                  ...
                  <Controller
                    name="answer"
                    control={control}
                    rules={{ required: selectedTypeName === 'Text' ? t("answer_is_required") : false }}
                    render={({ field }) => (
                      <input 
                        {...field}
                        type="text" 
