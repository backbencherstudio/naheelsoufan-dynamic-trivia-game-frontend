...
function QuestionsPage() {
  ...
  const { data, isLoading, isError } = useGetQuestionQuery({
    params: buildQueryParams(debouncedSearch),
  });
  ...
  const columns = [
    ...
    },
    {
      ...
      formatter: (value: any) => {
        ...
        return <span className="text-sm">{ans?.text}</span>;
      },
    },
    {
    ...
    },
    {
      ...
      formatter: (src: string) => {
        const kind = detectMediaKind(
          src,
          typeof src !== "string" ? (src as File | null) : null
        );
        return (
          <div>
            {src == null || src == "" ? (
              <span className="text-sm border rounded-sm bg-grayColor1/60 w-full h-17 flex items-center justify-center text-gray-500">
                No Image
              </span>
            ) : (
  ...
  const handleExportQuestions = () => {
    try {
      ...
      link.href = url;
      link.download = `questions_export_${
    ...
    try {
      ...
      input.onchange = async (e: any) => {
        ...
        try {
          ...
          if (res?.data?.success) {
            toast.success(
              res?.data?.message || "Questions imported successfully"
            );
          } else {
